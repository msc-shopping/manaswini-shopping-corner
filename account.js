...
function renderLocalOrders(user) {
  ...
  if (!box) return;
  if (!mine.length) {
    box.innerHTML = `<div class="empty">No orders linked to this account yet.<br><a class="text-link" href="../shop/">Start shopping →</a></div>`;
    return;
  }
...
async function loadGoogleOrders(idToken) {
  ...
  try {
    ...
    if(!orders.length){ box.innerHTML='<div class="empty">No orders found for this Google account.<br><a class="text-link" href="../shop/">Start shopping →</a></div>'; return; }
...
async function trackAccountOrder(event) {
  event.preventDefault();
  const input = document.getElementById("accountTrackInput");
  const box = document.getElementById("accountTrackResult");
  const orderId = String(input?.value || "").trim().toUpperCase();
  if (!orderId || !box) return;
  box.hidden = false;
  box.innerHTML = '<div class="spinner"></div> Checking your order...';
  try {
    const response = await fetch(`${ACCOUNT_API_URL}?action=trackOrder&orderId=${encodeURIComponent(orderId)}`, {cache:"no-store"});
    const data = await response.json();
    if (data.status !== "success" || !data.order) throw new Error(data.message || "Order ID not found.");
    const o = data.order;
    box.innerHTML = `<strong>${accountEsc(o.OrderID || orderId)}</strong><div class="account-track-grid"><span>Status</span><b>${accountEsc(o.OrderStatus || "Order Placed")}</b><span>Payment</span><b>${accountEsc(o.PaymentStatus || "Pending")}</b><span>Total</span><b>₹${Number(o.TotalAmount || 0).toLocaleString("en-IN")}</b></div>`;
  } catch (error) {
    box.innerHTML = `<span class="form-message">${accountEsc(error.message || "Unable to track this order.")}</span>`;
  }
}
