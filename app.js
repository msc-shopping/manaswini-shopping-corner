...
function productCard(p) {
  ...
  return `<article class="product-card" data-id="${escapeAttr(p.ProductID)}">
    <div class="product-image">
      ${img ? `<img src="${escapeAttr(img)}" alt="${escapeAttr(p.ProductName)}">` : `<div class="image-placeholder">M</div>`}
      ${moq > 1 ? `<span class="badge">MOQ ${moq}</span>` : ""}
    </div>
    <div class="product-info">
      <div class="product-cat">${escapeHtml(p.Category || "Collection")}</div>
      <h3 class="product-name">${escapeHtml(p.ProductName || "Product")}</h3>
      <div><span class="price">${money(price)}${p.Unit ? ` / ${escapeHtml(p.Unit)}` : ""}</span>${mrp > price ? `<span class="mrp">${money(mrp)}</span>` : ""}</div>
      <div class="moq">${moq > 1 ? `Minimum order: ${moq} ${escapeHtml(p.Unit || "units")}` : "Minimum order: 1"}</div>
      <div class="card-actions"><button class="small-btn">View</button><button class="small-btn primary" data-add>Add</button></div>
    </div>
  </article>`;
}
...
function openProduct(id) {
  ...
  $("#modalCategory").textContent = p.Category || "Collection";
  $("#modalName").textContent = p.ProductName || "Product";
  $("#modalPrice").textContent = `${money(num(p.Price))}${p.Unit ? ` / ${p.Unit}` : ""}`;
  $("#modalMOQ").textContent = getMOQ(p) > 1 ? `Minimum Order: ${getMOQ(p)}` : "MOQ: 1";
  $("#modalDescription").textContent = p.Description || "Product details will be updated soon.";
  setModalQty(state.modalQty);
  const img = String(p.ImageURL || "").trim();
  $("#modalImage").innerHTML = img ? `<img src="${escapeAttr(img)}" alt="${escapeAttr(p.ProductName)}">` : `<div class="image-placeholder">M</div>`;
...
function addToCart(p, quantity=1) {
  if (!p) return;
  const moq = getMOQ(p);
  let qty = Math.max(moq, Math.floor(quantity));
  const stock = num(p.Stock);
  if (stock > 0) qty = Math.min(qty, stock);
  const existing = state.cart.find(i => i.productId === String(p.ProductID));
  if (existing) existing.quantity += qty;
  else state.cart.push({productId:String(p.ProductID), productName:String(p.ProductName), quantity:qty, price:num(p.Price), unit:String(p.Unit||""), imageUrl:String(p.ImageURL||"")});
  saveCart();
  renderCart();
}
